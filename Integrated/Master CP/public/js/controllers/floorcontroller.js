egm.controller("floorController", function ($scope, API, $filter, $compile,$timeout) {
    ...
    $scope.addMobileSection = function () {
        ...
        if ($scope.selectedMobileSections.length == 0) {
            ...
            if (sectionPercent - 0.9 < actualPercent && actualPercent < sectionPercent + 0.9) {
                ...
                $scope.mobilefloor.Coordinates.push($scope.oneStoreMobileCoordinates);
                $scope.imgLink = '';
                //remove class active
                $('.active').removeClass('active');
                // clear array from isBusy status
    ...
    $scope.loadMobileArray = function () {
        ...
        for (var i = 0; i < $scope.mobilefloor.Coordinates.length; i++) {
            var top = $scope.mobilefloor.Coordinates[i].Top * oneSectionHeight;
            var left = $scope.mobilefloor.Coordinates[i].Left * oneSectionWidth;
            var height = $scope.mobilefloor.Coordinates[i].Height * oneSectionHeight;
            var width = $scope.mobilefloor.Coordinates[i].Width * oneSectionWidth;
            var div = document.createElement('div');
            div.innerHTML = '<div data-toggle="modal" data-target=".modalDeleteMobileSection" class="customMobileSectionDv" ng-click="deleteSection(' + $scope.mobilefloor.Coordinates[i].index + ')" style="cursor:pointer;background-image:url(' + $scope.mobilefloor.Coordinates[i].Img + ');position:absolute;top:' + top + ';left:' + left + ';height:' + height + ';width:' + width + ';background-size: cover;background-repeat: no-repeat;"></div>';
