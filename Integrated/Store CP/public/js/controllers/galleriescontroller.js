egm.controller("galleriesController", ['$scope', 'API', function ($scope, API) {
    ...
    $('.modal').on('hidden.bs.modal', function () {
        ...
        $scope.frmAddGallery.$setPristine();
        $scope.isEmpty = true;
        $scope.errMsg = false;
        $scope.errdiv = false;
    ...
    $scope.moveToGallery = function (_galleryId) {
        window.location.href = '/gallery/' + _galleryId;
    };
...
function convertCreateGalleryImgToBase64URL(event) {
